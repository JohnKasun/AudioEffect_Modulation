...
CAudioFileRaw::CAudioFileRaw() : CAudioFileIf(), m_piTmpBuff(0) {
  reset(true);
}
...
//////////////////////////////////////////////////////////////////////////////////////////
#ifdef WITH_SNDLIB
CAudioFileSndLib::CAudioFileSndLib() : CAudioFileIf(), m_FileHandle(-1), m_lFrameCnt(0), m_lFileLength(0), m_ppdTmpBuff(0) {
  reset(true);
}
...
Error_t CAudioFileSndLib::openFile(std::string cAudioFileName, FileIoType_t eIoType, FileSpec_t const *psFileSpec /*= 0*/) {
  ...
  if (getIoType() == kFileRead) {
    ...
    m_FileHandle =
        mus_sound_open_output(cAudioFileName.c_str(), CUtil::float2int<int>(sSpec.fSampleRateInHz), sSpec.iNumChannels,
